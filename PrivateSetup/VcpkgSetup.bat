...
Rem ECHO Update de Glad...
 CALL "%LOCAL_VCPKG_DIR%\vcpkg.exe" remove glad 
 CALL "%LOCAL_VCPKG_DIR%\vcpkg.exe" install glad 
 IF %ERRORLEVEL% NEQ 0 (
     ECHO Erreur lors de la mise à jour de glad .
     PAUSE
     EXIT /B 1
 )

Rem ECHO Update de glfw3...
...
Rem ECHO Installation de glad...
 CALL "%LOCAL_VCPKG_DIR%\vcpkg.exe" install glad
 IF %ERRORLEVEL% NEQ 0 (
     ECHO Erreur lors de l'installation de glad.
     PAUSE
     EXIT /B 1
 )

Rem ECHO Installation de glfw3...
 CALL "%LOCAL_VCPKG_DIR%\vcpkg.exe" install glfw3
 IF %ERRORLEVEL% NEQ 0 (
